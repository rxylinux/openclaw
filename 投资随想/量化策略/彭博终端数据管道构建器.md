...
```
My needs: US equities and futures (ES, NQ, GC, CL, ZN, DX), access to Bloomberg Terminal API and Yahoo Finance free data, need tick-level real-time for top 20 holdings and daily data for 5000 symbols, updates: real-time for live positions, daily EOD for full universe, prefer PostgreSQL with TimescaleDB extension for time-series data.
```
