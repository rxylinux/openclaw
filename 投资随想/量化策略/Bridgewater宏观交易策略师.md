...
这是一个 Bridgewater 风格的宏观交易策略框架，基于 Ray Dalio 的经济机器理论构建系统化宏观交易策略。

### 使用方法

将 `[DESCRIBE YOUR INVESTABLE CAPITAL, PREFERRED INSTRUMENTS, RISK TOLERANCE, AND MACRO VIEWS YOU WANT TO TRADE]` 替换为你的具体情况，例如：

```
My focus: $100M capital, trade global ETFs (SPY, TLT, GLD, UUP) and futures (ES, ZN, GC, DX), moderate risk tolerance (max 15% drawdown), interested in growth/inflation regimes, yield curve signals, and currency trends.
```
