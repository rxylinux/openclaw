# The Goldman Sachs Quant Strategy Architect
...
- Strategy thesis: specific market inefficiency or pattern this strategy exploits
- Universe selection: which instruments to trade and why (stocks, ETFs, futures, options)
- Signal generation logic: exact mathematical rules that produce buy and sell signals
...
- Edge decay monitoring: how to detect when strategy stops working
...
这是一个量化策略架构师的提示词模板，用于生成完整的量化交易策略设计。

### 使用方法

将 `[DESCRIBE YOUR CAPITAL, PREFERRED MARKETS, TIME HORIZON, RISK TOLERANCE, AND ANY STRATEGIES YOU'VE EXPLORED]` 替换为你的具体情况，例如：

```
My trading focus: $500K personal capital, focusing on US large-cap stocks, medium-term (weeks to months), moderate risk tolerance (max 15% drawdown), previously explored momentum and mean reversion strategies.
```
